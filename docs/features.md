...
| F-inline-band | Inline band | rho draws a fixed band at the bottom of the terminal. The band holds the live rows, the composer, and the footer. rho never enters the alternate screen, so the output above the band stays. | `rho-tui` | `planned` | No extension point. See `D-inline-viewport-not-alternate-screen`. |
| F-freeze-upward | Freeze a row upward | A row that can never change again moves into the terminal's own scrollback. So the wheel, a drag-select, and the terminal search all work on the transcript. | `rho-tui` | `planned` | No extension point. A row freezes only when it is final, and only in order. See `D-a-frozen-row-never-repaints`. |
