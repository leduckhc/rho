...
| F-session-list | Session list | `rho sessions list` prints six columns, newest first, inside 80 of them. The columns are the id, the last activity, the title, the model, the tokens, and the cost. `--long` adds the working directory and the fork origin. A row comes from a bounded head read and a bounded tail read. No whole file is decoded, and 500 sessions measure 20.68 ms. A file rho cannot read is one row marked unreadable, never a failed list. No column reports a turn count, because a count needs a whole file. `rho sessions show` prints one line per record with its id. | `rho-core`, `rho-cli` | `sprint-2` | `SessionStore::rows` and `row_from` are the seams. `rho-acp` reaches neither yet; `F-session-commands-over-acp` owns that. See `D-a-bad-session-file-is-one-row` and `docs/benchmarks.md`. |
