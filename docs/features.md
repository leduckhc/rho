...
| F-tool-result-handle | Tool result handle | A large tool result returns a bounded head preview, the retained byte count, and a session-scoped handle. The `read_tool_result` tool reads a byte range from that handle, or searches it for a literal string. | `rho-core`, `rho-tools` | `planned` | Replace by registering a different `Tool` under the name `read_tool_result`. The store behind it is a trait. See D-tool-result-handle. |
