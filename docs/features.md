...
| F-steer-subagent | Steer a subagent | The model sends a running child a new instruction. The child reads it at its next turn boundary, so it never lands inside a provider request. | `rho-tools` | `built` | Register a different `Tool` under the name `steer_agent`. |
