...
| F-mcp-schema-cache | MCP schema cache | A successful handshake writes the tool schemas to disk, so a later session advertises those tools with no wait. Without the write, `tools_for` reads an empty cache and no MCP tool ever reaches the model. | `rho-mcp`, `rho-cli` | `sprint-4` | Transparent to a server. See `docs/verification/mcp-live-probe.md`. |
...
| F-dead-surface-guard | Dead-surface guard | `bench/check-dead-surface.py` reports every public function in a crate that no code outside its own module and tests calls. `bench/allowed-uncalled.txt` exempts one function per line, with a mandatory reason, and a stale entry fails. Five shipped defects had this shape. | `bench` | `partial` | Add a line with its reason, and name the spec that will consume the code. See `D-the-dead-surface-allowlist-names-its-lane`. |
