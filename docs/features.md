...
| F-replay-scope | Replay scope | Only the current tool loop replays its reasoning. A block from before the last prompt stays in the transcript and never travels again. The prompt is append-only, so an old block would cost its bytes on every later turn. | `rho-provider-bedrock` | `delivered` | Measured as O(turns squared) before the fix. See `D-replay-only-the-current-loop`. |
