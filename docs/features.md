...
| F-session-list | Session list | `rho sessions list` prints six columns, newest first, inside 80 of them. The columns are the id, the last activity, the title, the model, the tokens, and the cost. `--long` adds the working directory and the fork origin. A row comes from a bounded head read and a bounded tail read. No whole file is decoded, and 500 sessions measure 20.68 ms. A file rho cannot read is one row marked unreadable, never a failed list. No column reports a turn count, because a count needs a whole file. `rho sessions show` prints one line per record with its id. | `rho-core`, `rho-cli` | `sprint-2` | `SessionStore::rows` and `row_from` are the seams. `rho-acp` maps `session/list` to it. See `D-a-bad-session-file-is-one-row` and `docs/benchmarks.md`. |
...
| F-session-crash-continue | Crash continue | A session with no close record is the one a crash left behind. `SessionStore::newest_open` finds it, and `rho run --continue` reopens it after a real SIGKILL. The reopen is stated on disk. rho offers nothing unasked yet: a user runs `--continue`. **A closed session is still resumable.** `--continue` takes the newest session either way. Only the crash offer needs a missing close. | `rho-core`, `rho-cli` | `partial` | `SessionStore::newest_open` is the crash offer, and `newest_resumable` is what `--continue` takes. See `D-continue-takes-the-newest-session-closed-or-not`. |
| F-session-delete | Session delete | `rho sessions delete <id-prefix>` removes one session file, every sidecar beside it, and its lock file. Every branch inside that file goes with it. A session forked from it is its own file, so it stays. The command says both, and it says that the bytes are not overwritten. | `rho-core`, `rho-cli` | `sprint-2` | `SessionStore::delete` is the seam. `rho-acp` maps `session/delete` to it. |
