...
| F-markdown-colour | Markdown as colour | A heading, a fence, a code line, a quote, a list marker, and a rule each take a colour. The punctuation is removed. Inline emphasis is not built, so its markers still show. | `rho-tui` | `partial` | Line level is built. Inline emphasis needs the run-level row contract in `SPEC-tui-markdown` phase 2. The subset is closed by design: a new element edits the scanner. See `D-markdown-line-level-first`. |
