...
| F-mcp-schema-cache | MCP schema cache | A successful handshake writes the tool schemas to disk, so a later session advertises those tools with no wait. Without the write, `tools_for` reads an empty cache and no MCP tool ever reaches the model. | `rho-mcp`, `rho-cli` | `planned` | Transparent to a server. See `docs/verification/mcp-live-probe.md`. |
...
| F-dead-surface-guard | Dead-surface guard | `bench/check-dead-surface.py` reports every public function in a crate that no code outside its own module and tests calls. `bench/allowed-uncalled.txt` exempts one function per line, with a mandatory reason, and a stale entry fails. Five shipped defects had this shape. | `bench` | `planned` | Add a line with its reason, and name the spec that will consume the code. See `D-the-dead-surface-allowlist-names-its-lane`. |
...
| ID | Name | Outcome | Owning crate | Status | Extension point |
|----|------|---------|--------------|--------|-----------------|
| F-provider-base-url | Provider base url | `base-url`, `RHO_BASE_URL`, and `--base-url` point the OpenAI-compatible client at another host. Ollama, vLLM, LiteLLM, and LM Studio then work with no new crate. A remote plain-`http` url is refused. A loopback one is allowed. A startup notice names the host the credential goes to, and an untrusted project file loses the key. | `rho-config` sets it, `rho-cli` applies it | `planned` | Point it at any OpenAI-compatible endpoint. See `SPEC-wire-the-dead-switches` and `D-a-provider-base-url-is-a-config-key`. |
| F-agent-discovery-switch | Agent discovery switch | `--no-agents`, `no-agents`, and `RHO_NO_AGENTS` stop the agent-definition search. `--no-skills` stops the skill search only, and no longer removes subagents in silence. A skipped definition is reported. | `rho-config`, `rho-cli` | `planned` | Two loaders, two switches, one shape. See `D-skills-and-agents-are-two-switches`. |
...
| F-reduced-motion | Reduced motion | `--no-motion`, `tui-motion = false`, or `RHO_REDUCE_MOTION=1` stops the sweep. The renderer consults `motion_enabled` before it animates, and the footer still names the state in words. An accessibility escape that the code documented and never called. | `rho-tui`, `rho-cli` | `planned` | One switch, three sources. See `D-motion-answers-to-one-switch`. |
