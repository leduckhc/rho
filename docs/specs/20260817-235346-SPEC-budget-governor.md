# SPEC-budget-governor — The budget governor

Status: draft. Blocked on `SPEC-subagents`, the subagent work, because a governor must cap a
whole agent tree and not one session.
Owning crate: `rho-core`, with the enforcement in a `Hook`.
Features: F-budget-caps restated and widened, plus a new row for the shared case.
...
faster, and `SPEC-subagents` makes a child cost about 247 KB, so the cheap thing to create is not
...
| Money | `Usage::cost_usd` | The charge the provider reported. **Never an estimate from a price table.** See decision D-measured-cost-and-cache. |
...
pub enum BudgetScope {
    ...
    Session,
    /// One session plus every subagent beneath it. See SPEC-subagents.
    Tree,
...
- **Around a model call**, which needs the request and response hook points that
  `SPEC-hooks-and-plugins` records as F-model-request-and-response-hooks. Until those land, the governor reads usage from the event
...
- **A limit is absent by default.** No cap unless a caller sets one, and the default is
  stated in the constructor. Decision D-no-four-argument-session-new removed a constructor that hid a choice.
...
- **Absent is not zero.** A provider that reports no charge must not read as free. Decision
  D-measured-cost-and-cache made `Usage::add` keep that distinction, and the governor must not undo it.
...
- `an_absent_cost_does_not_count_as_zero_spend` — the D-measured-cost-and-cache rule, restated where it is easy
  to break.
...
- **The first version is one turn late** on the model call, until F-model-request-and-response-hooks lands.
- **A money cap is only as good as the provider's reporting.** Two of the three report no
  charge today.
- **It does not stop a runaway shell command.** That is `bash`'s timeout and `SPEC-bash-sandbox`'s
  sandbox.

## 9. Out of scope

- Persisting spend across a restart.
- A per-user or per-tenant scope. That needs a tenancy model, and see decision D-no-cross-session-cache for
