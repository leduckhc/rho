...
Features: F-append-only-session-log, F-session-resume, F-session-branching, F-ephemeral-mode, F-pi-session-import.
...
whole reason for the parent pointer. See `ADR-session-format`.
...
version. It does not adopt pi's model. See decision D-own-session-format.
...
pub struct Entry {
    ...
    /// guess per line. See decision D-one-timestamp-format.
...
the command are in `ADR-jsonl-codec`. `sonic-rs` wins on encode and on a large untyped decode.
...
Selected numbers, from `ADR-jsonl-codec`, on a real 1848-record session and a 50000-record
corpus:
...
frontend already consumes `AgentEvents` from `Session::prompt`. See F-event-stream and `ADR-event-model`.
So a recorder folds those events into records. This needs no change to the existing
`Session` or `SessionConfig`. See decision D-recorder-consumes-events.
...
opt-out for F-ephemeral-mode. The config key `ephemeral` and the absence of `session-file` both
select `Off`. See `SPEC-config`.
...
new function in `rho-redact`. This is decision D-redact-tool-arguments.
...
function exists.** `rho-redact` is the one home for redaction, per decision D-one-redaction-home, so no
...
The append path never rewrites the file. This is F-append-only-session-log. An earlier record is already
...
`bash` line reader turned 8 MB of output into 805 MB of memory. See decision D-bash-line-cap.
...
reader in decision D-bash-line-cap.
...
only. This is F-session-branching.
...
The ACP methods in the table above are not all live yet. `SPEC-acp` marks `session/load`,
...
not the trait-object comparison that `SPEC-subagents` section 3 proved impossible. It compares
...
open. See decision D-plugin-does-not-classify-itself.
...
client forks by a load followed by a new. See `SPEC-acp`.
...
Feature F-pi-session-import lives in the crate `rho-session-import-pi`. It converts a pi session file
...
- A record type this build does not know drops, and the count names the type. The import
  does not stop. See decision D-unmappable-pi-record-drops.
...
- A compaction or summary of an old branch. F-context-compaction and F-branch-summary own that.
- A search index over many sessions.
- A human-set session title, written or read cheaply. `list` gives no title.
- Adding `simd-json`. It lost on small records. See `ADR-jsonl-codec`.
