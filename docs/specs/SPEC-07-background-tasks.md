# SPEC-07 — Background tasks and progress reporting

Status: draft for sprint 2.
Owning crates: `rho-core` (the task registry and events), `rho-tools` (the `bash`
tool and the task tools).
Features: F-07 (background tasks), and new features for auto-backgrounding, progress
...
kill. See decision D-012 for why the boundary fails closed.
...
- **One line.** Cap at `max_line_bytes`, default 65536, and split a longer line.
  This is decision D-016. A newline-free stream once drove 805 MB of resident memory.
...
- The credential scrub of decision D-019 applies unchanged.
