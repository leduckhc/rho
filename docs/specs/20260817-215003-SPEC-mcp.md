# SPEC-mcp — Model Context Protocol client
...
- Connect lazily and register the tools when they arrive. The tool list changes
  ...
  prompt cache. `SPEC-core-runtime` section 1 forbids exactly this.
...
- **A server does not classify its own tools.** MCP has no `ToolKind`. So every MCP tool
  ...
  read-only policy denies it. This follows decision D-plugin-does-not-classify-itself, where a plugin's own claim
...
- **Output is untrusted.** Cap it, and sanitise it before it reaches the TUI, exactly
  like tool output.
- **A line is capped**, as in decision D-bash-line-cap, where an uncapped line reader reached
  805 MB of resident memory.
- **The environment is scrubbed** for a stdio server, as in decision D-bash-scrubs-credentials. A server
...
Security:
- `every_mcp_tool_reports_tool_kind_other` — the D-plugin-does-not-classify-itself rule. A server never classifies
  itself.
