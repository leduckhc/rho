# SPEC-approval — Approval and permission
...
Features: F-ask-approval-policy, F-approval-mode-resolution, F-permission-over-acp, F-approval-ui-extensions.
...
- `docs/specs/20260817-170455-SPEC-acp.md` covers the ACP permission path. ACP has
  `session/request_permission`.
- Sprint 2 ships subagents with `BothPolicies`, in `docs/specs/20260818-000223-SPEC-subagents.md`. A
...
## 5. The config keys, and how they interact with SPEC-config

`SPEC-config` owns the `approval` key. The value set is `read-only`, `ask`, and `allow-all`.
...
`ToolKind::Other` lesson from decision D-plugin-does-not-classify-itself, applied to the memory.
...
- A `cancelled` outcome maps to `Deny`, and the run stops with the stop reason
  `cancelled`. `SPEC-acp` section 5 owns the spelling.
...
## 9. The interaction with BothPolicies from SPEC-subagents

A child stays at most as permissive as its parent. `SPEC-subagents` section 3 enforces this by
...
unrepresentable, which is the point of `SPEC-subagents` section 3.
...
- A policy rule language, or a remote approval service. F-approval-ui-extensions owns those.
...
Row F-tool-approval-gate once claimed that the basic TUI wires a confirmation prompt to the approval gate.
`crates/rho-tui/src/` holds no approval code, so that claim was false. The current row no
longer carries the claim, and it must stay that way. F-tool-approval-gate should describe only the trait
...
- F-ask-approval-policy, the Ask approval policy, in `rho-core`. It stays `planned` until the policy is
  built.
- F-approval-mode-resolution, the approval mode resolution, in `rho-core` and `rho-cli`. It stays `planned`.
- F-permission-over-acp, permission over ACP, in `rho-acp`. It stays `planned`.
- F-approval-ui-extensions, the approval UI extensions, in `rho-tui`. It stays `planned`.

A new row is needed for the TUI Ask prompt itself, because F-approval-ui-extensions covers only the richer
