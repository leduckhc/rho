# SPEC-acp — ACP frontend mapping

Status: draft for sprint 1. Delivery is `planned`, after the TUI. The spec is
written now because it constrains the `rho-core` event model (decision D-acp-is-real-acp).

Owning crate: `rho-acp`. It depends on `rho-core`. It links no provider crate.

Decision D-acp-is-real-acp: `rho-acp` speaks the real Agent Client Protocol, the same protocol
...
Features covered: F-acp-frontend (ACP frontend), F-prompt-command (prompt), F-steer-command (steer), F-abort-command (abort),
F-session-commands-over-acp (session commands), F-extension-ui-sub-protocol (permission and extension UI).
...
same framing as the plugin protocol in `SPEC-hooks-and-plugins`.
...
| `session/load` | Planned. Needs the session format (F-append-only-session-log). |
| `session/prompt` | Map `prompt` content blocks to `Vec<ContentBlock>`; call `Session::prompt`; stream updates; reply with a `stopReason`. F-prompt-command. |
| `session/cancel` | Call `CancelToken::cancel` on the running turn. F-abort-command. |
...
Steering (F-steer-command) maps to a second `session/prompt` while a run is active. Sprint 1
...
chosen to match. This is why `SPEC-core-runtime` uses these exact names.
...
`SPEC-core-runtime` section 9 carries a `serde(rename = "cancelled")` attribute for this
...
calls. `rho-acp` provides an `ApprovalPolicy` (from `SPEC-tool-interface`) whose `approve`
...
A denied call becomes an error `ToolResult`, per `SPEC-core-runtime` and `SPEC-tool-interface`. The
...
- `promptCapabilities`: `image: false`, `audio: false`, `embeddedContext: false`
  ...
  Sprint 1 advertises `image: false`. The reason is honesty. `SPEC-provider-interface` section 8
...
`SPEC-core-runtime` section 10 lists the same gaps from the core side.

- `plan` updates: no plan or todo feature in sprint 1 (F-todo-tool is planned).
...
- `session/set_mode`, `session/list`, `session/resume`, `session/delete`,
  `session/close`, and `session/load`: planned. See F-session-commands-over-acp.
- The extension UI sub-protocol beyond permission (F-extension-ui-sub-protocol): planned.
...
- Session persistence methods: load, list, resume, delete, close (F-session-commands-over-acp).
- Session modes and `session/set_mode`.
- MCP server capabilities and terminal embedding.
- The extension UI sub-protocol beyond permission (F-extension-ui-sub-protocol).
- Full steer and follow-up queue semantics (F-steer-command is partial in sprint 1).
