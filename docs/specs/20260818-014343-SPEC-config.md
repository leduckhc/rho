# SPEC-config — Layered configuration

Status: draft for sprint 2.
Owning crate: `rho-config`.
Features: F-layered-config, F-environment-variable-override, F-credential-resolution, F-profile-support, F-no-secrets-in-logs.
...
default that grants more access than the file asked for. Decision D-plugin-does-not-classify-itself shipped the
...
pub struct Config {
    ...
    /// from the table in `SPEC-approval` section 4, which yields `ask` where a human or a
...
| `session-file` | path | The session file in `SPEC-sessions`. New config key. |
| `ephemeral` | bool | Ephemeral mode in `SPEC-sessions`. New config key. |
...
| `profiles` | table | New. The named profiles for F-profile-support. |
...
from the mode resolution in `SPEC-approval`: rho defaults to `ask` where a human or a client
...
D-bash-os-sandbox. A sandbox that breaks a build is worse than none, so `off` is the stated default
...
decides. So there is one gate, not two. See decisions D-plugin-does-not-classify-itself and D-config-fails-closed.
...
decision D-one-redaction-home. No code in `rho-config` writes a credential to `tracing`, not even at
`trace` level. Redaction is by construction. The type carries the mask, so no filter
must remember to run. This satisfies F-no-secrets-in-logs.
...
- A file larger than 1 MiB returns `ConfigError::TooLarge`. The **read** is bounded, and
  not only the value the read keeps. A project file arrives with a clone, so its size is
  attacker-controlled, and the read happens before the trust gate, because the gate reads
  the parsed file to know what to strip. See `D-a-config-file-is-read-under-a-cap`.

**A parse failure never falls back to a more permissive default.** This is the exact
defect that decision D-plugin-does-not-classify-itself fixed for `ToolKind::Other`. There, an untyped value read as
...
- `an_unreadable_file_is_a_read_error` — a file with no read permission returns
  `ConfigError::Read`.
- `a_file_over_the_cap_is_refused_and_names_the_limit` — a file one byte over 1 MiB returns
  `ConfigError::TooLarge`, naming the file and the limit.
- `a_file_at_the_cap_is_accepted` — a file exactly at the cap is still read, so the cap
  refuses one byte and not one byte less.
- `a_source_with_no_end_is_refused_and_the_read_ends` — the bound is on the read. A file
  cannot prove this, because a test file is as small as the cap, so the test reads an
  endless source and requires the read to end. Removing the `take` bound while keeping the
  length check leaves the two tests above green and fails this one alone.
...
- A custom `CredentialResolver` closure for a third-party source. F-credential-resolution lists it as a
  later extension point.
- A `ConfigSchema` for a third-party key set. F-layered-config lists it as planned.
...
- A model registry loaded from the file. F-model-registry owns that, and it is planned.
