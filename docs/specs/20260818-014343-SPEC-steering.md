...
///
/// It counts the text of every block, the base64 payload of an image, and the strings
/// inside tool-call arguments. `push` measures a message with this, so a caller can
/// measure the same way before it pushes.
...
payload of an image, and the strings inside tool-call arguments. So a large body cannot hide
in a block the count skips.
...
- `the_counted_size_covers_every_block_kind` — text, a base64 image, tool-call arguments,
  and a nested tool result all count, so a large body cannot hide in a skipped block.
...
- `every_constructor_carries_a_byte_cap` — `new`, `with_capacity`, and `default` all cap a
  message, so no constructor opts out.
