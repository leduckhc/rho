...
Features covered: F-agent-loop (agent loop), F-event-stream (event stream), F-turn-model (turn model),
F-cancellation (cancellation), F-stable-prefix-for-kv-cache (stable prefix), F-short-system-prompt (short system prompt),
F-library-first-api (library-first API). The event model is also constrained by the ACP
frontend; see `SPEC-acp` and section 12 below.

Decision note (D-acp-is-real-acp): the event model must express every concept the Agent
...
`ADR-event-model` for why. The provider maps its own wire format onto these variants.
...
- Tool definitions that appear or reorder after turn one. The full tool list is
  ...
  prefix. See `SPEC-hooks-and-plugins`.
...
client errors. See `SPEC-provider-interface` for the retry policy that reads `is_retryable`.
...
`ToolError` is defined in `SPEC-tool-interface`.
...
`ToolOutput` and `ToolKind` are defined in `SPEC-tool-interface`.
...
against a root that nobody chose. See decision D-session-config. A caller that wants the
...
`ApprovalPolicy` is defined in `SPEC-tool-interface`.
...
impl Session {
    ...
    /// The only constructor. Decision D-no-four-argument-session-new deleted a four-argument `new`, because
    ...
    /// of the messages, not a borrow. See decision D-session-context-accessor.
    pub async fn messages(&self) -> Vec<Message>;
}
```

`ToolRegistry` is defined in `SPEC-tool-interface`. `HookChain` and `Hook` are defined in
`SPEC-hooks-and-plugins`. `Provider`, `ProviderStream`, `CompletionRequest`, and `ToolSpec` are
defined in `SPEC-provider-interface`.
...
2. Consult the `ApprovalPolicy` from `SessionConfig`. It reads the tool `kind`,
   ...
   `ToolResult` and the tool never runs. See `SPEC-tool-interface`. In the ACP frontend the
...
table states the mapping. `SPEC-acp` gives the full detail. Where a concept has no
...
| tool `kind` | `ToolKind` on the tool and the `ToolSpec` (`SPEC-tool-interface`) |
| `session/request_permission` | `ApprovalPolicy` (`SPEC-tool-interface`), which is async |
...
- `plan` updates need a todo or plan feature (F-todo-tool, planned). Sprint 1 sends no
  plan updates.
...
## 11. Session format (planned, F-append-only-session-log)

Decision D-own-session-format: rho defines its own append-only JSONL session format. It is not
...
## 12a. Credentials and retry, from decision D-secret-in-core

`rho-core` owns two types that `SPEC-provider-interface` describes in detail.
...
decision D-secret-in-core for the drift that forced the move.
