...
Features covered: F-tool-trait (tool trait), F-file-read-tool to F-bash-tool (the built-in tools), F-path-confinement
(path confinement), F-tool-approval-gate (approval gate). The tool `kind` and the async approval
policy exist so `rho-acp` can report ACP tool kinds and drive
`session/request_permission`; see `SPEC-acp`.
...
/// `rho-acp` forwards the value with no remap. See `SPEC-acp`.
...
prefix stable, per `SPEC-core-runtime`.
...
also block a call; see `SPEC-hooks-and-plugins`. The policy runs after the hooks.
...
selected outcome back to `Allow` or `Deny`. See `SPEC-acp`.
...
- `read` (kind `read`) — read a file. Args: `path`, optional `offset`, optional
  `limit`. Returns text. Read-only. F-file-read-tool.
- `write` (kind `edit`) — write a whole file. Args: `path`, `content`. Creates
  parent directories. Mutating. Needs approval. F-file-write-tool.
- `edit` (kind `edit`) — replace an exact text span in a file. Args: `path`,
  ...
  `similar` to build a diff for the result. Mutating. Needs approval. F-file-edit-tool.
- `list` (kind `read`) — list a directory. Args: `path`. Returns entries.
  Read-only. F-directory-list-tool.
- `glob` (kind `search`) — match files by a glob pattern under the root. Args:
  `pattern`. Uses `globset`. Read-only. F-glob-tool.
- `grep` (kind `search`) — search file contents by a regex. Args: `pattern`,
  ...
  `.gitignore`. Read-only. F-grep-tool.
- `bash` (kind `execute`) — run a shell command. Args: `command`, optional
  `timeout_ms`. Streams output. Mutating. Needs approval. See section 7. F-bash-tool.
...
**A second, real boundary is now available: the OS sandbox.** `SPEC-bash-sandbox` adds a
...
refuses the command. See `SPEC-bash-sandbox` for what the sandbox does and does not stop.
...
in `SPEC-bash-sandbox` is the honest limit: it is off by default, and it does not stop a read.
...
In `crates/rho-core/src/tool.rs` unit tests, for `confine` (F-path-confinement) and the
policies (F-tool-approval-gate):
...
- A container sandbox for `bash`. The OS sandbox in `SPEC-bash-sandbox` uses
  `sandbox-exec` or `bwrap`, not a container.
- A network allow-list for `bash`. `strict` in `SPEC-bash-sandbox` is all or nothing.
