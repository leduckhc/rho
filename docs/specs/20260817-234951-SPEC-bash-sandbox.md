# SPEC-bash-sandbox — bash OS sandbox
...
Feature: F-bash-os-sandbox (bash OS sandbox).

## 1. The problem this solves

`SPEC-tool-interface` section 7 states the truth: path confinement does not apply to `bash`.
...
records 100 concurrent live sessions in 34 MiB, and `SPEC-subagents` for what that unlocks.
...
sandbox. The default is stated in the code, not hidden. See decision D-no-four-argument-session-new.
...
- **Unprivileged `unshare` is not a backend.** A robust write confinement with
  ...
  boundary while a single path walked through it, which decision D-bash-no-path-confinement forbids. So
...
- A per-command or per-path allow list finer than the session root. That is the
  `rho-guard` extension, F-scoped-command-guardrails.
