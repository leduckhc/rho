...
formats normalise onto the one event model from `SPEC-core-runtime`. This spec defines the
trait, the request type, the three mappings, the retry policy, and the secret
redaction rule.

Features covered: F-provider-trait (provider trait), F-openrouter-provider (OpenRouter), F-aws-bedrock-provider (Bedrock),
F-azure-openai-provider (Azure), F-auto-retry (auto-retry), F-no-secrets-in-logs (no secrets in logs), F-separate-feature-flags-for-providers (per-provider
feature flags).
...
> `rho-core`, and every provider imports them. See decision D-secret-in-core. They were
