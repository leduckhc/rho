...
Features covered: F-minimal-tui (minimal TUI), F-pure-function-render (pure-function render), F-non-blocking-input
(non-blocking input, Ctrl-C behaviour), F-time-to-first-frame (time to first frame).
...
## 6. Time to first frame (F-time-to-first-frame)
...
no number. See decision D-benchmarks-owner.
...
- `cli_definition_is_valid`, `build_config_fails_without_a_model`,
  ...
  and the explicit `SessionConfig` construction. See decision D-no-four-argument-session-new.

## 8. Out of scope for sprint 1

- Themes (F-themes) and keybinding config (F-keybindings).
- A custom tool renderer (F-custom-tool-renderer).
- Scrollback search. Mouse input was out of scope for sprint 1, and a later stage added
  one click: it runs the slash row under the pointer. See `D-a-panel-nobody-can-open`.
