...
`docs/features.md` F-hook-trait-tier-1 to F-plugin-schema-cache.
...
- Tier 1: in-tree Rust traits (`Provider`, `Tool`, `Hook`). Zero process
  overhead. See `SPEC-provider-interface`, `SPEC-tool-interface`, `SPEC-hooks-and-plugins`.
- Tier 2: out-of-process plugins over stdio JSON-RPC. Any language. Crash
  isolated. See `SPEC-hooks-and-plugins`.
...
- Con: binary-size cost. A general WASM engine such as Wasmtime adds several
  ...
  footprint budget in `ADR-footprint`.
...
- The plugin schema cache (F-plugin-schema-cache) keeps the prompt prefix stable when a plugin
  connects late. See `SPEC-hooks-and-plugins` section 5.
- The core carries no WASM dependency. The footprint budget in `ADR-footprint` holds.
