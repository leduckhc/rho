# ADR-footprint — Async runtime and footprint
...
optimisations. See F-low-resident-memory-per-session, F-fast-cold-start, F-release-build-size, F-separate-feature-flags-for-providers.
...
- No WASM engine. See `ADR-plugin-mechanism`.
...
- No provider crate in the core. Providers are optional cargo features on
  `rho-cli` (F-separate-feature-flags-for-providers), so a minimal build links one provider only.
...
records them, with the command, in `docs/benchmarks.md` (decision D-benchmarks-owner). This
