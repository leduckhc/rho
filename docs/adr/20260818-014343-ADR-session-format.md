# ADR-session-format — Session file format
...
ACP `session/load` and `session/resume` both need a stored format. See F-append-only-session-log to F-pi-session-import and
`SPEC-sessions`.

The format has hard constraints.

- A third party reads the file directly. F-append-only-session-log promises this, so the format stays plain.
...
record that names an earlier record as its parent. See `SPEC-sessions`.
...
- A third party reads the file with any JSONL reader. The format needs no rho library.
  This keeps the F-append-only-session-log promise real, not aspirational.
- The shape matches what pi writes, so the import path in `SPEC-sessions` section 9 is a
...
- A record repeats its structural keys on every line. JSONL is not the smallest
  encoding. The record-size cap in `SPEC-sessions` section 3 bounds the worst case, and the
  format stays readable, which the smaller encodings lose.
- The codec choice matters for throughput. `ADR-jsonl-codec` measures it, and `SPEC-sessions` makes
...
- A branch needs a copy, or an in-document tree that a third party cannot read simply.
  So it breaks the F-append-only-session-log promise and the cheap-branch requirement at once.
...
- It adds a C dependency and a file lock. That is heavier than the footprint budget in
  `ADR-footprint` allows for a core that holds 50 sessions.
...
- A third party can no longer read the session with a plain reader. It needs the
  database engine and the schema. So it breaks F-append-only-session-log.
...
- This is decision D-own-session-format, and it stands. rho keeps its own record set, and a one-way
...
- The first record is the header, and it carries a `version`. A reader refuses a version
  ...
  `SPEC-sessions` guards this, so `SessionError::Version` is proved and not just described.
- The header also carries the resolved `approval` and `sandbox` mode names. A resume
  ...
  See `SPEC-sessions` section 8a. This keeps a stored permission from silently widening under a
  live config.
...
- A codec change is invisible. Both codecs produce a byte-identical line for a record,
  and each reads the other's output. See `SPEC-sessions` section 3 and `ADR-jsonl-codec`.

## Consequences

- The record set in `SPEC-sessions` is the stable on-disk contract. A change to it is a
...
- The session log is a consumer of the event stream, not a field in `Session`. So the
  format and the runtime stay separate, and either can change alone. See `SPEC-sessions`
  section 5.
